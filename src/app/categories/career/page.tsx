...
export default function CareerCategoryPage() {
  const articles = getPostsByCategory("Career");

  return (
    <CategoryPageAnimations>
    <div className="max-w-full category-career">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <CategoryBanner category="Career" />
        <div className="mb-12">
          ...
          <JsonLd id="breadcrumb-schema" data={{
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 2, name: "Career", item: "https://www.insightnerd.in/categories/career" },
            ],
          }} />
          <h1 className="text-4xl font-bold mb-4">Career</h1>
          <p className="text-xl text-muted-foreground">
            Career transitions, job hunting, and professional growth in tech
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {articles.map((article) => (
            <Link key={article.slug} href={`/posts/${article.slug}`} className="article-card career-card">
              {article.cover_image && (
