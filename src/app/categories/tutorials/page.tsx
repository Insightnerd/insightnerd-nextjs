...
export default function TutorialsCategoryPage() {
  const articles = getPostsByCategory("Tutorials");

  return (
    <CategoryPageAnimations>
    <div className="max-w-full category-tutorials">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <CategoryBanner category="Tutorials" />
        <div className="mb-12">
          ...
          <JsonLd id="breadcrumb-schema" data={{
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 2, name: "Tutorials", item: "https://www.insightnerd.in/categories/tutorials" },
            ],
          }} />
          <h1 className="text-4xl font-bold mb-4">Tutorials</h1>
          <p className="text-xl text-muted-foreground">
            Step-by-step guides that work in production
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {articles.map((article) => (
            <Link key={article.slug} href={`/posts/${article.slug}`} className="article-card tutorials-card">
              {article.cover_image && (
